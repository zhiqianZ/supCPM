...
classdef QfHiDM < handle
    ...
    methods
        ...
        function [trainedTestEvents, trainingMap, dissimilarities, ...
            ...
            trainingMap=Map;
            for i=1:N
                trainer=bestTrainer4TestClass(i);
                tIdx=tIdxs(i);
                sId=sIds_(i);
                ...
                if trainer>0
                    trainingMap.add(num2str(trainer), i);
                ...
                trainedTestEvents(sIdPer==sId)=trainer;
            end
        end
        
        function getFalsePosNegs(this, verbose)
            out={};
            html='';
            if ~isequal(this.tData, this.sData)
                this.falsePosNegHtml='';
                this.falsePosNegs={};
                return;
            end
            [trainedTestEvents, trainingMap, dissimilarities, fMeasures]=...
                this.matchTestEvents(nargin==1||verbose);
            this.falsePosEvents=trainedTestEvents;
            trainingEvents=this.tIdPerRow(:,1);
            N=length(this.tIds);
            precisions = zeros(1,N);
            recalls = zeros(1,N);
            for i=1:N
                trainingId=this.tIds(i);
                r=struct;
                r.trainingClass=String.ToHtmlSupFromTex(this.tNames{i});
                r.trainingId=trainingId;
                r.trainingSize=this.tSizes(i);
                r.truth=sum(trainingEvents==trainingId & trainedTestEvents==trainingId);
                r.falsePos=sum(trainingEvents~=trainingId & trainedTestEvents==trainingId);
                r.falseNeg=sum(trainingEvents==trainingId & trainedTestEvents~=trainingId);
                ...
                if nMatches==1
                    sIdx=sIdxs{1};
                    sName=String.ToHtml(this.sNames{sIdx});
                    r.testIds=this.sIds(sIdx);
                    r.testSize=this.sSizes(sIdx);
                ...
                elseif nMatches>1
                    ...
                    for j=1:nMatches
                        sIdx=sIdxs{j};
                        sName=String.ToHtml(this.sNames{sIdx});
                        testIds(j)=this.sIds(sIdx);
                        testSize=testSize+this.sSizes(sIdx);
            ...
            if N>0
                td='<td align="right">';
                html=['<table cellpadding="2" border="1"><thead><tr>'...
                ...
                for i=1:N
                    r=out{i};
                    html=[html '<tr>'...
                        '<td>' r.trainingClass '</td>'...
                        '<td>' num2str(r.trainingId) '</td>'...
                        td String.encodeInteger(r.trainingSize) '</td>'];
                    if r.testIds(1)>0
                        html=[html ...
                            '<td>' StringArray.toString(...
                            r.testClasses, '<br>') '</td>'...
                            '<td>' String.Num2Str(r.testIds, '<br>') '</td>'...
            ...
            this.falsePosNegHtml=html;
            this.falsePosNegs=out;
            this.falsePosNegsPlot=prp;
            this.falsePosEvents=trainedTestEvents;
        end
        
        function [accurateIdxs, falsePosIdxs, falseNegIdxs, actualIdxs]...
                =getPredictions(this, trainingId)
            falsePosIdxs=find(this.tIdPerRow(:,1)~=trainingId &...
                this.falsePosEvents==trainingId);
            falseNegIdxs=find(this.tIdPerRow(:,1)==trainingId & ...
                this.falsePosEvents~=trainingId);
            accurateIdxs=find(this.tIdPerRow(:,1)==trainingId & ...
                this.falsePosEvents==trainingId);
            actualIdxs=find(this.tIdPerRow(:,1)==trainingId);
        end
    ...
    methods(Access=private)
        ...
        function [f, overlap, p, r]=fMeasureInBins(this, tIdSet, sIdSet)
            tChoices=MatBasics.LookForIds(this.tIdPerRow, tIdSet);
            sChoices=MatBasics.LookForIds(this.sIdPerRow, sIdSet);
            [f, overlap, p, r]=this.adaptiveBins.fMeasure(tChoices, sChoices);
        end
