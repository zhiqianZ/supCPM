...
classdef QfTable < handle
    ...
    methods
        function this=QfTable(qf, tClrs, props, priorFig, visible)
            ...
            try
                if isempty(this.qf.falsePosNegs)
                    this.qf.getFalsePosNegs;
                end
                if ~isempty(this.qf.falsePosNegs)
                    ToolBarMethods.addButton(tb_, ...
                        fullfile(path, 'plusMinus.png'), ...
                        'See false positive/negative results in browser', ...
        ...
        function browseFalsePosNeg(this)
            img=Html.TempImg(this.qf.falsePosNegsPlot);
            Html.BrowseString(Html.Wrap(['<h2> False positives'...
                ' &amp; negatives on same data</h2>' img '<hr>' ...
                this.qf.falsePosNegHtml '<hr>' img ]));
        ...
        function doHistF(this, visible, showFalsePosNeg)
            if nargin<3
            ...
            if ishandle(this.fHistFig)
                ...
                this.fHistFig=fig_;
                if showFalsePosNeg && ~isempty(this.qf.falsePosNegsPlot)
                    op=get(fig_, 'outerposition');
                    set(fig_, 'outerposition', [op(1) op(2) op(3)*1.5 op(4)*.85]);
                    Gui.Locate(fig_, this.fig, 'south east++');
                    
                    ax=subplot(1,2,1,'parent', fig_);
                else
                    Gui.Resize(fig_, .66);
                    Gui.Locate(fig_, this.fig, 'south east+');
                    ax=axes('parent', fig_);
                ...
                if showFalsePosNeg && ~isempty(this.qf.falsePosNegsPlot)
                    ...
                    file=[tempname '.png'];
                    try
                        saveas(this.qf.falsePosNegsPlot, file);
                    catch ex
                    end
                    ax=subplot(1,2,2,'parent', fig_);
                    image(ax, imread(file));
                    set(ax, 'xticklabel', [], 'yticklabel', []);
                    title(ax, 'False positive/negative');
                    width=.1;
                    lbl='See +/-  details';
                else
                    set(ax, 'units', 'normalized', 'Position', ...
                ...
                if ~isempty(this.qf.falsePosNegsPlot)
        ...
        function QF=save(this, qf, file) 
            ...
            QF.tClrs=this.tClrs;
            QF.falsePosNegsPlot=qf.falsePosNegsPlot;
            QF.falsePosNegs=qf.falsePosNegs;
            QF.falsePosNegHtml=qf.falsePosNegHtml;
        ...
        function closeFigs(this)
            closeFig(this.fig);
            closeFig(this.qHistFig);
            closeFig(this.fHistFig);
