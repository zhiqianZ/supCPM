...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    stop_lying_iter = 1;                              % iteration at which lying about P-values is stopped
    ...
    for iter=1:max_iter
        ...
        if iter <300 && iter >1
         ...
         for j=1:10
             g = randn(size(ydata));
             grad1 = grad1+ grad(ydata-t*g,P)*exp(-norm(t*g)^2*4/1e-8/300);
             a = a+exp(-norm(t*g)^2*4/1e-8/300);
         end
       % ydata = ydata - 200*grad1/a;
        y_grads=grad1/a;
