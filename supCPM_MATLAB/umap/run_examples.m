...
function run_examples(whichOnes)
N_EXAMPLES = 19;
...
if ismember(6, whichOnes)
    ...
    [~,~, clusterIds]=run_umap('sample30k.csv', 'cluster_output', 'graphic', 'verbose', verbose);
...
if ismember(15, whichOnes)
    ...
    run_umap('sampleRag148k.csv', 'template_file', 'ustBalbc2D.mat', 'verbose', verbose);
...
if ismember(15, whichOnes)
    ...
    run_umap('sampleRag148k.csv', 'template_file', 'pyUstBalbc2D.mat', 'verbose', verbose);
...
if ismember(16, whichOnes)
    ...
    [reduction, umap, clusterIdentifiers,extras]=run_umap('sample10k.csv', 'template_file', 'ustBalbc3D.mat', 'qf_tree', true, 'qf_dissimilarity', true, 'see_training', true, 'cluster_output', 'graphic', 'verbose', verbose);
    disp('run_umap Example 16 completed with no MATLAB exceptions!');
end
if ismember(17, whichOnes)
    disp('run_umap Example 17 starting...');
    run_umap('sampleBalbcLabeled12k.csv', 'template_file', 'ustBalbc2D.mat', 'label_column', 'end', 'match_scenarios', 4, 'verbose', verbose, 'see_training', true, 'color_file', 'colorsByName.properties');
    disp('run_umap Example 17 completed with no MATLAB exceptions!');
end
if ismember(18, whichOnes)
    disp('run_umap Example 18 starting...');    
    run_umap('sampleBalbcLabeled12k.csv', 'template_file', 'ustBalbc2D.mat', 'label_column', 'end', 'match_scenarios', 4, 'match_histogram_fig', false, 'see_training', true, 'verbose', verbose, 'false_positive_negative_plot', true);
    disp('run_umap Example 18 completed with no MATLAB exceptions!');
end
if ismember(19, whichOnes)
    disp('run_umap Example 19 starting...');    
    run_umap('s2_samusikImported_29D.csv', 'template_file', 'ust_s1_samusikImported_29D_30nn_3D.mat', 'label_column', 'end', 'label_file', 's2_samusikImported_29D.properties', 'match_scenarios', 4,  'match_histogram_fig', false, 'see_training', true, 'false_positive_negative_plot', true, 'match_supervisors', [3 1 4], 'verbose', verbose);
