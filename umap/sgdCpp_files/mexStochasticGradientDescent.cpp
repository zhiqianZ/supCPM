...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if (nlhs != 1){
        if (nrhs<13){
            mexErrMsgTxt("Why call if you don't listen? 0 argout and <13 argin (epochCallbackFcn)");
            return;
        }
    }
/* sanity check: ensure we have input data */
    if (nrhs != 12 && nrhs != 13) {
        mexPrintf("Expecting 12 or 13 args NOT %d args!\n", nrhs);
        mexErrMsgTxt("Expected args: head_embedding, tail_embedding, head, tail, n_epochs, n_vertices, epochs_per_sample, a, b, gamma, initial_alpha, negative_sample_rate, @(data,epochs)fcnProgress");
        return;
    }
