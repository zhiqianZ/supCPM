...
classdef UMAP_extra_results < handle
    ...
    methods
        ...
        function doFalsePosNeg(this)
            ...
            lf=newline;
            tab=sprintf('\t');
            head=sprintf(['reduction\tsampleSet\ttrainingSet\ttestSet\t'...
                'neighbors\thiD\tloD\tmatchType\tclusterDetail']);
            
            head=[ head tab ...
                'trainingClass' tab 'trainingId' tab 'trainingSize' tab...
                'testClasses' tab 'testIds' tab 'testSize' tab...
                'fMeasure' tab 'falsePos' tab 'falsePosRatio' tab...
                'falseNeg' tab 'falseNegRatio' tab 'qfDissimilarity' lf];
            body=this.falsePosNegBody;
            this.falsePosNegFile=[UMAP_extra_results.FileName(args_, 'falsePosNeg') '.txt'];
            ...
            for i=1:N
                ...
                if N2>0
                    ...
                    context=sprintf(...
                        ...
                        testSet, args_.n_neighbors, args_.hiD, ...
                        args_.n_components, mt, cd);

                    for j=1:N2
                        rec=qt.qf.falsePosNegs{j};
                    ...
                    if ~isempty(notFound)
                        body=[body lf notFound];
                    end
                    body=[body lf];
                end
            end
            this.falsePosNegHead=head;
            this.falsePosNegBody=body;
        end
        
        function html=getFalsePosNegHtml(this, h1, forBrowser)
            if nargin<3
                forBrowser=true;
                if nargin<2
                    h1='';
            ...
            for i=1:N
                ...
                if N2>0
                    pngFile=File.SwitchExtension(this.falsePosNegFile, '.png');
                    [pngFldr, pngFn, pngExt]=fileparts(pngFile);
                    saveas(qf.qf.falsePosNegsPlot, pngFile);
                    html=['<html>' h1 '<table><tr><td>' ...
                        Html.Img([pngFn pngExt], pngFldr, 1, forBrowser) ...
                        '</td><td>' qf.qf.falsePosNegHtml...
                        '</td></tr></table></html>'];
                    return;
                end
            end
            html='<html>No data for false positive/negatives<br>match_scenarios==4 is required.</html>';
        end
        
        function seeFalsePosNegs(this, forBrowser)
            if nargin<2
                forBrowser=true;
            end
            html=this.getFalsePosNegHtml(...
                '<h2>False positives &amp; negatives</h2>', forBrowser);
            if ~forBrowser
                obj.javaWindow=Gui.JFrame(this.fig);
                obj.msg=html;
                msg(obj, 0, 'north east++', 'False positive & negatives');
            else
